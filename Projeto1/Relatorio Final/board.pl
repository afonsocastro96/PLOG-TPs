...
game_cvp :- once(ask_board), !, once(pick_tower), bot_pick_colour(Colour), assert(bot_colour(Colour)), game_cycle(Winner), end_game(Winner).
...
pick_tower :- display_board, write('Player 1: State the vertical coordinate of the first white tower: (Ex: a.)'), read(Character), write('State the horizontal coordinate of the first white tower: (Ex: 1.)'), read(Number), validate_pick_tower(Character, Number), pick_tower2.
validate_pick_tower(Character, Number) :- integer(Number), board_length(Length), Number =< Length, pick_tower_aux(Character, Number, 'L').
validate_pick_tower(_,_) :- write('Invalid tower placement!'), nl, nl, pick_tower.
pick_tower2 :- display_board, write('Player 1: State the vertical coordinate of the second white tower: (Ex: a.)'), read(Character),  write('State the horizontal coordinate of the second white tower: (Ex: 1.)'), read(Number), validate_pick_tower2(Character, Number), pick_tower3.
validate_pick_tower2(Character, Number):- integer(Number), board_length(Length), Number =< Length, pick_tower_aux(Character, Number, 'L').
validate_pick_tower2(_,_) :- write('Invalid tower placement!'), nl, nl, pick_tower2.
pick_tower3 :- display_board, write('Player 1: State the vertical coordinate of the first black tower: (Ex: a.)'), read(Character),  write('State the horizontal coordinate of the first black tower: (Ex: 1.)'), read(Number), validate_pick_tower3(Character, Number), pick_tower4.
validate_pick_tower3(Character, Number) :- integer(Number), board_length(Length), Number =< Length, pick_tower_aux(Character, Number, 'T').
validate_pick_tower3(_,_) :- write('Invalid tower placement!'), nl, nl, pick_tower3.
pick_tower4 :- display_board, write('Player 1: State the vertical coordinate of the second black tower: (Ex: a.)'), read(Character),  write('State the horizontal coordinate of the second black tower: (Ex: 1.)'), read(Number), validate_pick_tower4(Character, Number).
validate_pick_tower4(Character, Number) :- integer(Number), board_length(Length), Number =< Length, pick_tower_aux(Character, Number, 'T').
...
make_play_aux(Move) :- Move == 'pass', pass, once(change_player).
...
move_tower :- 	write('\nState the vertical coordinate of the tower you want to move: (Ex: a.)'), read(Character),
					...
					char_code(Character,Charcode), write('\n'), Y is Charcode-97, X is Number-1,
					char_code(NCharacter,NCharcode), write('\n'), NY is NCharcode-97, NX is NNumber-1,
					move_tower_play(X,Y,NX,NY).

slide_tile_play(X,Y,NX,NY) :- slide_tile_aux(X,Y,NX,NY), current_player(Player), reset_pass(Player).
sink_tile_play(X,Y) :- sink_tile_aux(X,Y), current_player(Player), reset_pass(Player).
move_tower_play(X,Y,NX,NY) :- move_tower_aux(X,Y,NX,NY), current_player(Player), reset_pass(Player).

slide_tile_aux(X,Y,NX,NY) :- valid_slide(X,Y,NX,NY), board_cell(X,Y,Elem), change_tile(NX,NY,Elem), change_tile(X,Y,[' ', ' ', ' ']), once(change_player).
slide_tile_aux(_,_,_,_) :- write('Invalid move!'), nl, nl.
sink_tile_aux(X,Y) :- valid_sink(X,Y), board_cell(X, Y, [' ',C,S]), remove_colour_shape(C,S), change_tile(X,Y,[' ', ' ', ' ']), once(change_player).
sink_tile_aux(_,_) :- write('Invalid move!'), nl, nl.
move_tower_aux(X,Y,NX,NY) :- valid_move(X,Y,NX,NY), board_cell(X,Y,[Tower|_]), insert_tower(NX, NY, Tower), remove_tower(X,Y), once(change_player).
